...
<script setup lang="ts">
  ...
  import { preConnect } from '@/api/socket'
  // 补充导入 onMounted（你原有代码已隐含，这里明确写出确保可用）
  ...
  /**
   * 页面挂载完成（打开时）执行：调用预连接接口 + 按需自动连接WebSocket
   */
  onMounted(async () => {
    // 1. 监听页面可见性变化（保留你原有逻辑）
    ...
    // 2. 调用预连接接口（核心：页面打开时自动执行）
    try {
      addLog('info', '开始执行预连接接口')
      // 若 preConnect 是异步接口（返回 Promise），必须用 await 接收返回值
      const preConnectData = await preConnect(1)
      console.log('预连接接口返回数据：', preConnectData.server)
      // 4. 从预连接数据中提取服务器地址
      connectForm.value.url = preConnectData.server

      addLog('success', '预连接接口调用成功')

      // 3. 可选：预连接成功后，自动触发 WebSocket 连接（无需手动点击按钮）
      // 如果你不想自动连接，删除这一行即可
      handleConnect()
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      addLog('error', `预连接接口调用失败：${errorMessage}`)
      ElMessage.error('预连接失败，请稍后重试')
    }
  })
</script>
<style scoped>
